...
public class FeedbackHistoryFragment extends Fragment implements RetrofitListener<FeedbackModel> {
    RecyclerView rv_historyFeed;
    LinearLayout noFeed;
    ...
    void init(View view) {
        rv_historyFeed = view.findViewById(R.id.rv_historyFeedback);
        noFeed = view.findViewById(R.id.ll_noFeedback);
        pullToRefresh = view.findViewById(R.id.pullToRefresh);
        noFeed.setVisibility(View.VISIBLE);
        rv_historyFeed.setVisibility(View.GONE);
    ...
    void loadFeed(int page) {
        Util.checkInternet(requireActivity(), new Util.NetworkCheckCallback() {
            ...
            public void onNetworkCheckComplete(boolean isAvailable) {
                if (isAvailable) {
                    apiServiceProvider.FeedbackList(LOGIN_DETAIL.getAppuserID(), Constant.HISTORY_FEED_STATUS, Constant.FEED_PAGE_LIMIT, String.valueOf(page), FeedbackHistoryFragment.this);
                } else {
                    hideLoader();
                }
            }
    ...
    public void onResponseSuccess(FeedbackModel successResponse, String apiFlag) {
        if (successResponse.statusCode == 200) {
            ...
            if (newData != null && newData.size() > 0) {
                ...
                rv_historyFeed.setVisibility(View.VISIBLE);
